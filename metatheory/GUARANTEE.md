...
2. **`axion_rt.c`** — ~1900 LOC of hand-written C implementing all reclamation primitives
   (`axion_*_free`, bignum `bn_divmod`, arrays, strings, arena, session runtime, `parMap`). Trusted;
   checked only by sanitizers + fuzz. *The bignum `bn_divmod` double-free lived here, below the verified
   line.* Mitigation target: verify or shrink/generate from a checked spec (north star).
