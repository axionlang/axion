...
for f in AxionDrop.lean AxionAlias.lean AxionKey.lean; do
  OUT="$("${LEAN[@]}" "$f" 2>&1)"
  status=$?
  echo "$OUT"
  if [ $status -ne 0 ]; then
...
echo "OK: AxionDrop + AxionAlias + AxionKey metatheory check (no sorry; axioms = propext/Quot.sound only)"
