...
| Future | Per-field ownership (`%1` fields with own Δ — the strict-ANF form of the paper's constructor component typing); revisit `Field`-ret as owned (kills the Field-split rule); bridge to dependent sorts (§7-4 of memory-model-options.md). | **designed**: docs/per-field-ownership.md (F-1..F-4 phases; Field-split rule at core.rs `transfers_heap_field`/`Elab::case_arms` collapses into remainder drops). |
