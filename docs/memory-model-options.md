...
1. **Phase A′ — narrow annotations first.** Attach `Type` **and its usage
   environment Δ** (the Linear-Core annotation, §3.1) to the allocation/return/
   `%1`-signature nodes (`Make*`, call results, `sigs`), and to each `Drop` node.
   This is the minimal IR change; the backends stay untouched.
2. **Migrate Auto-Drop bottom-up and delete the heuristics as they are subsumed:**
   each step must be verified by `scripts/sanitize.sh` (ASan/LSan: `allocs == frees`,
   no double-free) and `scripts/differential.sh` (GHC oracle: same verdicts, zero
   semantic drift) before moving to the next — the drop path is the most
   memory-safety-critical code in the compiler.
...
|---|---|---|---|
| **Annotate Core nodes + thread types from the AST** | New, but small (~300 lines) | Low | ✅ the first cut — the smallest safe step |
| **Heuristics** (`core.rs:2454–3370`, ~900 lines) | Deleted as subsumed | Negative | ✅ net-negative — precisely the point of this document |
| **Backends, runtime, surface checks** | Untouched | Zero | ✅ nothing to do |
| **Δ validator (full Linear Core)** | New, heavy (~1–2k lines) | High | ⚠️ the final *completion*, not step 1 |
