...
-- `axion_drop_List$Maybe$P` (frees the `Just` boxes and the `P` payloads).
-- build 3 → 3 Cons + 3 Just + 3 P = 9 allocs, all freed; = 1.
data P = P { a :: Int }

build :: Int -> List (Maybe P)
build n = if n == 0 then Nil else Cons (Just (P { a = n })) (build (n - 1))
