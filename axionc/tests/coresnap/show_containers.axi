

























                      drop _t27 : List$List$Int
                      drop _t28 : String
                      let _d1000000 = putStrLn _t28  ; Δ{_t28}
                      let _t20 = con Nil  ; Δ{} · makes List$Int
                      let _t21 = con Cons 1 _t20  ; Δ{_t20} · moves{_t20} · makes List$Int
                      let _t22 = con Nil  ; Δ{_t21} · makes List$Int
                      let _t23 = con Cons 3 _t22  ; Δ{_t21 _t22} · moves{_t22} · makes List$Int
                      let _t24 = con Cons 2 _t23  ; Δ{_t21 _t23} · moves{_t23} · makes List$Int
                      let _t25 = con Nil  ; Δ{_t21 _t24} · makes List$List$Int
                      let _t26 = con Cons _t24 _t25  ; Δ{_t21 _t24 _t25} · moves{_t24 _t25} · makes List$List$Int
                      let _t27 = con Cons _t21 _t26  ; Δ{_t21 _t26} · moves{_t21 _t26} · makes List$List$Int
                      let _t28 = call show$List$List$Int _t27  ; Δ{_t27} · makes String
                      ret _d1000000  ; Δ{}
                    _ ->
                  drop _t18 : String
                  let _t17 = con TPlus  ; Δ{}
                  let _t18 = call show$Trit _t17  ; Δ{} · makes String
                  let _t19 = putStrLn _t18  ; Δ{_t18}
                  ret case _t19 of
                _ ->
              drop _t15 : String
              let _t14 = con LT  ; Δ{}
              let _t15 = call show$Ordering _t14  ; Δ{} · makes String
              let _t16 = putStrLn _t15  ; Δ{_t15}
              ret case _t16 of
            _ ->
          drop _t11 : List$Maybe$Int
          drop _t12 : String
          let _t10 = con Cons _t8 _t9  ; Δ{_t7 _t8 _t9} · moves{_t8 _t9} · makes List$Maybe$Int
          let _t11 = con Cons _t7 _t10  ; Δ{_t10 _t7} · moves{_t10 _t7} · makes List$Maybe$Int
          let _t12 = call show$List$Maybe$Int _t11  ; Δ{_t11} · makes String
          let _t13 = putStrLn _t12  ; Δ{_t12}
          let _t7 = con Just 1  ; Δ{} · makes Maybe$Int
          let _t8 = con Nothing  ; Δ{_t7} · makes Maybe$Int
          let _t9 = con Nil  ; Δ{_t7 _t8} · makes List$Maybe$Int
          ret case _t13 of
        _ ->
      drop _t0 : String
      drop _t0 : String
      drop _t0 : String
      drop _t0 : String
      drop _t0 : String
      drop _t0 : String
      drop _t0 : String
      drop _t1 : String
      drop _t1 : String
      drop _t1 : String
      drop _t1 : String
      drop _t1 : String
      drop _t1 : String
      drop _t1 : String
      drop _t2 : String
      drop _t2 : String
      drop _t2 : String
      drop _t4 : Maybe$Int
      drop _t5 : String
      drop xs
      drop xs
      let _d1000000 = rtcall axion_strcat ", " _t2  ; Δ{_t2} · makes String
      let _d1000000 = rtcall axion_strcat ", " _t2  ; Δ{_t2} · makes String
      let _d1000000 = rtcall axion_strcat ", " _t2  ; Δ{_t2} · makes String
      let _d1000000 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _d1000000 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _d1000000 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _d1000000 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _dd0 = loadraw _p+16  ; Δ{}
      let _dd0 = loadraw _p+16  ; Δ{}
      let _dd0 = loadraw _p+16  ; Δ{}
      let _dd0 = loadraw _p+16  ; Δ{}
      let _dd1 = call axion_drop_List _dd0  ; Δ{}
      let _dd1 = call axion_drop_List$Int _dd0  ; Δ{}
      let _dd1 = call axion_drop_List$List$Int _dd0  ; Δ{}
      let _dd1 = call axion_drop_List$Maybe$Int _dd0  ; Δ{}
      let _dd2 = loadraw _p+8  ; Δ{}
      let _dd2 = loadraw _p+8  ; Δ{}
      let _dd3 = call axion_drop_List$Int _dd2  ; Δ{}
      let _dd3 = call axion_drop_Maybe$Int _dd2  ; Δ{}
      let _t0 = call double y  ; Δ{y ys}
      let _t0 = call show$Int y  ; Δ{} · makes String
      let _t0 = call show$Int z  ; Δ{} · makes String
      let _t0 = call show$List$Int y  ; Δ{} · makes String
      let _t0 = call show$List$Int z  ; Δ{} · makes String
      let _t0 = call show$Maybe$Int y  ; Δ{} · makes String
      let _t0 = call show$Maybe$Int z  ; Δ{} · makes String
      let _t0 = rtcall axion_strcat "Just" " "  ; Δ{} · makes String
      let _t1 = call map$$double ys  ; Δ{y ys} · moves{ys} · makes List$Int
      let _t1 = call showArg$Int a0  ; Δ{_t0} · makes String
      let _t1 = call showListRest$Int ys  ; Δ{_t0} · makes String
      let _t1 = call showListRest$Int zs  ; Δ{_t0} · makes String
      let _t1 = call showListRest$List$Int ys  ; Δ{_t0} · makes String
      let _t1 = call showListRest$List$Int zs  ; Δ{_t0} · makes String
      let _t1 = call showListRest$Maybe$Int ys  ; Δ{_t0} · makes String
      let _t1 = call showListRest$Maybe$Int zs  ; Δ{_t0} · makes String
      let _t2 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _t2 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _t2 = rtcall axion_strcat _t0 _t1  ; Δ{_t0 _t1} · makes String
      let _t4 = con Just 3  ; Δ{} · makes Maybe$Int
      let _t5 = call show$Maybe$Int _t4  ; Δ{_t4} · makes String
      let _t6 = putStrLn _t5  ; Δ{_t5}
      ret ""  ; Δ{}
      ret ""  ; Δ{}
      ret ""  ; Δ{}
      ret ""  ; Δ{}
      ret ""  ; Δ{}
      ret ""  ; Δ{}
      ret "EQ"  ; Δ{}
      ret "GT"  ; Δ{}
      ret "LT"  ; Δ{}
      ret "Nothing"  ; Δ{}
      ret "TMinus"  ; Δ{}
      ret "TPlus"  ; Δ{}
      ret "TZero"  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret 0  ; Δ{}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
      ret case _t6 of
      ret con Cons _t0 _t1  ; Δ{_t1 y} · moves{_t1} · makes List$Int
      ret con Nil  ; Δ{} · makes List$Int
    Cons y ys ->
    Cons y ys ->
    Cons y ys ->
    Cons y ys ->
    Cons z zs ->
    Cons z zs ->
    Cons z zs ->
    EQ ->
    GT ->
    Just a0 ->
    LT ->
    Nil ->
    Nil ->
    Nil ->
    Nil ->
    Nil ->
    Nil ->
    Nil ->
    Nothing ->
    TMinus ->
    TPlus ->
    TZero ->
    _ ->
    else
    else
    else
    else
    let _dd2 = == _tag 1  ; Δ{}
    let _dd2 = == _tag 1  ; Δ{}
    let _dd3 = if _dd2 then
    let _dd3 = if _dd2 then
    let _dd4 = == _tag 1  ; Δ{}
    let _dd4 = == _tag 1  ; Δ{}
    let _dd5 = if _dd4 then
    let _dd5 = if _dd4 then
    let _dfree = rtcall axion_free _p  ; Δ{}
    let _dfree = rtcall axion_free _p  ; Δ{}
    let _dfree = rtcall axion_free _p  ; Δ{}
    let _dfree = rtcall axion_free _p  ; Δ{}
    let _dfree = rtcall axion_free _p  ; Δ{}
    let _t1 = + lo 1  ; Δ{}
    let _t1 = + lo 1  ; Δ{}
    let _t1 = + lo 1  ; Δ{}
    let _t2 = + acc lo  ; Δ{}
    let _t2 = call range _t1 hi  ; Δ{} · makes List$Int
    let _t2 = callclo c lo n  ; Δ{}
    let _tag = loadraw _p+0  ; Δ{}
    let _tag = loadraw _p+0  ; Δ{}
    let _tag = loadraw _p+0  ; Δ{}
    let _tag = loadraw _p+0  ; Δ{}
    let _tag = loadraw _p+0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret 0  ; Δ{}
    ret acc  ; Δ{}
    ret call rangeFused _t1 hi c _t2  ; Δ{}
    ret call rangeFusedSum _t1 hi _t2  ; Δ{}
    ret con Cons lo _t2  ; Δ{_t2} · moves{_t2} · makes List$Int
    ret con Nil  ; Δ{} · makes List$Int
    ret n  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  ; Δ{}
  drop _t0 : String
  drop _t0 : String
  drop _t0 : String
  drop _t1 : List$Int
  drop _t1 : String
  drop _t1 : String
  drop _t1 : String
  drop _t2 : String
  else
  else
  else
  else
  else
  else
  else
  else
  let _d1000000 = rtcall axion_strcat "[" _t1  ; Δ{_t1} · makes String
  let _d1000000 = rtcall axion_strcat "[" _t1  ; Δ{_t1} · makes String
  let _d1000000 = rtcall axion_strcat "[" _t1  ; Δ{_t1} · makes String
  let _dd0 = band _p 1  ; Δ{}
  let _dd1 = if _dd0 then
  let _dd4 = band _p 1  ; Δ{}
  let _dd4 = band _p 1  ; Δ{}
  let _dd5 = if _dd4 then
  let _dd5 = if _dd4 then
  let _dd6 = band _p 1  ; Δ{}
  let _dd6 = band _p 1  ; Δ{}
  let _dd7 = if _dd6 then
  let _dd7 = if _dd6 then
  let _t0 = > lo hi  ; Δ{}
  let _t0 = > lo hi  ; Δ{}
  let _t0 = > lo hi  ; Δ{}
  let _t0 = call range 1 5  ; Δ{} · makes List$Int
  let _t0 = call showListElems$Int xs  ; Δ{} · makes String
  let _t0 = call showListElems$List$Int xs  ; Δ{} · makes String
  let _t0 = call showListElems$Maybe$Int xs  ; Δ{} · makes String
  let _t1 = call map$$double _t0  ; Δ{_t0} · moves{_t0} · makes List$Int
  let _t1 = rtcall axion_strcat _t0 "]"  ; Δ{_t0} · makes String
  let _t1 = rtcall axion_strcat _t0 "]"  ; Δ{_t0} · makes String
  let _t1 = rtcall axion_strcat _t0 "]"  ; Δ{_t0} · makes String
  let _t2 = call show$List$Int _t1  ; Δ{_t1} · makes String
  let _t3 = putStrLn _t2  ; Δ{_t2}
  ret + x x  ; Δ{}
  ret 0  ; Δ{}
  ret 0  ; Δ{}
  ret 0  ; Δ{}
  ret 0  ; Δ{}
  ret 0  ; Δ{}
  ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
  ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
  ret _d1000000  ; Δ{_d1000000} · moves{_d1000000}
  ret case _t3 of
  ret case x of
  ret case x of
  ret case x of
  ret case xs of
  ret case xs of
  ret case xs of
  ret case xs of
  ret case ys of
  ret case ys of
  ret case ys of
  ret if _t0 then
  ret if _t0 then
  ret if _t0 then
  ret rtcall axion_array_free _p  ; Δ{}
  ret showInt x  ; Δ{} · makes String
  ret showInt x  ; Δ{} · makes String
axion_drop_Array _p  =
axion_drop_List _p  =
axion_drop_List$Int _p  =
axion_drop_List$List$Int _p  =
axion_drop_List$Maybe$Int _p  =
axion_drop_Maybe$Int _p  =
double x  =
main  =
map$$double xs  =
range lo hi  =
rangeFused lo hi c n  =
rangeFusedSum lo hi acc  =
show$Int x  =
show$List$Int xs  =
show$List$List$Int xs  =
show$List$Maybe$Int xs  =
show$Maybe$Int x  =
show$Ordering x  =
show$Trit x  =
showArg$Int x  =
showListElems$Int xs  =
showListElems$List$Int xs  =
showListElems$Maybe$Int xs  =
showListRest$Int ys  =
showListRest$List$Int ys  =
showListRest$Maybe$Int ys  =
